...
evalE env (EBin o e1 e2) = evalE env e1 >>= \n1 ->
                           evalE env e2 >>= \n2 ->
...
evalE env (EIf c t e) =
  case eval env c of
    ...
    _ -> error "Condition must evaluate to a boolean value"


evalE env (ELet x e1 e2) = evalE env e1 >>= \n1 ->
                           evalE ((x, n1):env) e2

evalE env (EApp e1 e2) =
  case evalE env e1 of
    Right (VClos frozenEnv x body) ->
      let
        v = eval env e2
...
evalE env (ETry e1 x e2)
  | Left ex1 <- evalE env e1 = evalE ((x, ex1) : env) e2
  | Right ex1 <- evalE env e1 = Right ex1
...
prelude :: Env
prelude =
  [ ("tail", VPrim primTail),("head", VPrim primHead)
